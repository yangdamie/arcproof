...
export default function App() {
  ...
  const [jobs, setJobs] = useState<ServiceJob[]>(loadJobs);
  const [activeJob, setActiveJob] = useState<ServiceJob | undefined>(() => loadJobs()[0]);
  const [showCreate, setShowCreate] = useState(false);
  const [showDeliverable, setShowDeliverable] = useState(false);
  const [busy, setBusy] = useState(false);
  const [toast, setToast] = useState<Toast | undefined>();

  const isLive = Boolean(configuredEscrowAddress && isAddress(configuredEscrowAddress));
  ...
  async function onCreate(input: {
    ...
    try {
      if (isLive) {
        ...
        if (!isAddress(input.provider)) throw new Error("Provider wallet must be a valid address.");
        if (input.evaluator && !isAddress(input.evaluator)) throw new Error("Evaluator wallet must be a valid address.");
      }

      const localId = Math.max(0, ...jobs.map((job) => job.id)) + 1;
      let id = localId;
      let txHash: string | undefined;

      if (isLive) {
        const result = await createOnchainJob({
      ...
      const newJob: ServiceJob = {
        ...
        evaluator: input.evaluator || (wallet ? shortAddress(wallet) : "You"),
      ...
      setJobs((all) => [newJob, ...all.filter((job) => job.id !== newJob.id)]);
      setActiveJob(newJob);
      setShowCreate(false);
      setView("jobs");
      setToast({ type: "success", message: isLive ? `Job #${id} created on Arc Testnet.` : "Demo job created locally." });
    } catch (error) {
  ...
  <svg viewBox="0 0 36 36" role="img">
    ...
    <text x="18" y="29.5" textAnchor="middle">A</text>
        ...
        <nav className="nav">
          <NavItem icon="O" label="Overview" active={view === "dashboard"} onClick={() => setView("dashboard")} />
          <NavItem icon="J" label="Jobs" active={view === "jobs"} onClick={() => setView("jobs")} count={metrics.active} />
          <NavItem icon="E" label="Evidence vault" active={view === "evidence"} onClick={() => setView("evidence")} />
          <NavItem icon="S" label="Settings" active={view === "settings"} onClick={() => setView("settings")} />
        </nav>
      ...
      <main className="main">
        ...
        {view === "settings" && <SettingsView isLive={isLive} onReset={() => { localStorage.removeItem("arcproof.jobs"); setJobs(demoJobs); setActiveJob(demoJobs[0]); setToast({ type: "info", message: "Demo data restored." }); }} />}
      </main>
...
function Dashboard({ jobs, metrics, onOpen, onViewJobs, onFund, busy }: {
  ...
  return <section className="page-content">
    <section className="hero-card">
      ...
      <div className="proof-orbit" aria-hidden="true"><div className="orbit orbit-one" /><div className="orbit orbit-two" /><div className="proof-core">OK<small>Proof</small></div><div className="orbit-label label-a">USDC held</div><div className="orbit-label label-b">Hash stored</div><div className="orbit-label label-c">Release</div></div>
    </section>

    <section className="metric-grid">
      <Metric label="USDC in escrow" value={`${metrics.locked.toLocaleString()} USDC`} hint="Funds locked in active agreements" mark="$" />
      <Metric label="Settled volume" value={`${metrics.paid.toLocaleString()} USDC`} hint="Completed jobs in this workspace" mark="OK" />
      <Metric label="Active jobs" value={String(metrics.active).padStart(2, "0")} hint="Awaiting delivery or approval" mark="A" />
...
function Timeline({ active, label, note }: { active: boolean; label: string; note: string }) {
  return <div className={`timeline-step ${active ? "timeline-active" : ""}`}><span className="timeline-dot">{active ? "OK" : ""}</span><div><strong>{label}</strong><small>{note}</small></div></div>;
}
...
function Modal({ title, subtitle, onClose, children }: { title: string; subtitle: string; onClose: () => void; children: React.ReactNode }) {
  return <div className="modal-backdrop" onMouseDown={onClose}><section className="modal" onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" onClick={onClose}>x</button><span className="eyebrow">ARCPROOF AGREEMENT</span><h2>{title}</h2><p>{subtitle}</p>{children}</section></div>;
}
